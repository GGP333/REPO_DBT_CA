...
|   |-- figures/                      # per-dataset loss curves (generated locally, not tracked)
|
|-- docs/
...
- Training by-products: the per-case sample dumps, the per-run learning curves and the
  per-dataset loss figures. They are regenerated by the training and plotting scripts. They were
  removed from the repository because 123 of them buried the figures that matter; the figures the
  manuscript uses are kept, under `results/paper_assets/`, `results/ablation/figures/`,
  `results/statistics/figures/` and `docs/figures/`.
...
### 9.5 Where the figures and tables are

The two figures of the revision are the ablation figure, in `results/ablation/figures/`, and the
paired per-patient figure that replaces Figure 3, in `results/statistics/figures/`. Both are
tracked in PNG and in vector PDF. The earlier manuscript figures — training curves, segmentation
examples and score comparisons — are under `results/paper_assets/`.
