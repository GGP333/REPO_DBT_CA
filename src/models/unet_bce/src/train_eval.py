...
def validate_one_epoch(model: torch.nn.Module,
    ...
    for batch in tqdm(loader, desc="Val", leave=False):
        ...
        # Si hubo padding, recortar a shape original
        shapes = batch["shapes"]
...
def fit(model: torch.nn.Module,
        ...
        preds_out_dir: str = "outputs/preds_npy") -> Dict[str, Any]:
    """
    ...
    for epoch in range(1, max_epochs+1):
        ...
        val_metrics = validate_one_epoch(model, val_loader, device, threshold=float(cfg.threshold), preds_out_dir=preds_out_dir)
        ...
        try:
            ...
            tqdm.write(f"Epoch {epoch:03d}/{int(cfg.max_epochs)} | "
                       ...
                       f"val_f1={val_metrics['f1']:.4f} | val_acc={val_metrics['accuracy']:.4f} | lr={optimizer.param_groups[0]['lr']:.6f}")
        except Exception:
        ...
        torch.save({"model_state": model.state_dict(),
                    ...
                    "epoch": epoch,
                    "val_dice": val_metrics["dice"]}, last_path)
        ...
        if improved:
            ...
            torch.save({"model_state": model.state_dict(),
                        "cfg": vars(cfg) if hasattr(cfg, "__dict__") else dict(cfg),
                        "epoch": epoch,
                        "best_val_dice": best_dice}, best_path)
            es_counter = 0
        else:
            if es_enabled:
                es_counter += 1
...
@torch.no_grad()
def evaluate_from_checkpoint(model_ctor, ckpt_path: str, val_loader: DataLoader,
                             threshold: float = 0.5,
                             preds_out_dir: str = "outputs/preds_npy") -> Dict[str, float]:
    """
    ...
    model.to(device)
    model.eval()
    from train_eval import validate_one_epoch  # reuse
    metrics = validate_one_epoch(model, val_loader, device, threshold=threshold, preds_out_dir=preds_out_dir)
    return metrics
