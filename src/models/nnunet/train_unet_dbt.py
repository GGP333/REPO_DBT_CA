#!/usr/bin/env python3
"""
Script de entrenamiento/evaluación para nnUNet3D auto-configurado.

Flujo con auto_plan=True:
  1. discover_studies → fingerprint → plan → nnUNet3D.from_plan(plan)
  2. PatchDBTDataset para entrenamiento (patches aleatorios con fg oversampling)
  3. DBTVolumeDataset + sliding window inference para validación/test
  4. Post-procesamiento de connected components

Flujo legacy (auto_plan=False):
  Comportamiento original (volúmenes completos, arquitectura fija).
"""
...
# Shared utilities
_shared_dir = str(Path(__file__).resolve().parent / "Models" / "shared") if not (Path(__file__).resolve().parent.parent / "shared").exists() else str(Path(__file__).resolve().parent.parent / "shared")
# Resolve properly: train_unet_dbt.py is in Models/nnUnet_original/
_shared_dir = str(Path(__file__).resolve().parent.parent / "shared")
if _shared_dir not in sys.path:
    sys.path.insert(0, _shared_dir)

from config_loader import load_config
from dataset_dbt import DBTVolumeDataset, PatchDBTDataset, collate_pad, collate_patches, set_global_seeds
...
from fingerprint import compute_fingerprint, save_fingerprint
from planner import generate_plan, save_plan


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Entrenamiento nnUNet3D auto-configurado en DBT (.npy).")
    ...
    parser.add_argument("--base-ch", type=int, default=32, help="Canales base del modelo (legacy).")
    parser.add_argument("--levels", type=int, default=4, help="Niveles de la U (legacy).")
...
def _normalize_ratios(train_ratio: float, val_ratio: float, test_ratio: float) -> Tuple[float, float, float]:
    ...
    if total > 1.0:
        warnings.warn(
            f"Ratios suman {total:.2f}>1; se renormalizan para que sumen 1.",
            stacklevel=2,
        )
        train_ratio /= total
        val_ratio /= total
...
def make_loader(records: List[StudyRecord], cfg, shuffle: bool) -> DataLoader:
    """Loader legacy: volúmenes completos con padding."""
...
def make_patch_loader(
    ...
    """Loader auto-configurado: patches aleatorios de tamaño fijo."""
    patch_size = tuple(plan["patch_size"])
    batch_size = plan.get("batch_size", 1)
    fg_prob = float(plan.get("oversample_foreground", 0.33))

    # Override batch_size desde CLI si se proporcionó
    if hasattr(cfg, "batch_size") and cfg.batch_size:
        batch_size = int(cfg.batch_size)

    ds = PatchDBTDataset(
        records,
        patch_size=patch_size,
        force_fg_prob=fg_prob,
        augment=augment,
        cfg=cfg,
        logger=get_logger("patch_loader"),
    )
    return DataLoader(
        ds,
...
def plot_paper_curves(log_csv: Path, out_dir: Path) -> None:
    ...
    for (col, label, color), ax in zip(metrics_main, axes):
        if col not in df.columns:
            ...
            continue
        _plot_with_band(ax, epochs, df[col], label, color)
    fig.suptitle("Curvas de entrenamiento/validación (nnUNet Auto)", fontsize=14, fontweight="bold")
...
def main() -> None:
    ...
    logger = get_logger("train_unet_dbt_script")
    ...
    # ========== Auto-configuración vs Legacy ========== #
    auto_plan = bool(getattr(cfg, "auto_plan", True))
    plan: Optional[Dict[str, Any]] = None

    if auto_plan:
        logger.info("=" * 60)
        logger.info("MODO AUTO-CONFIGURACIÓN (nnU-Net self-adapting)")
        logger.info("=" * 60)

        # 1. Fingerprint del dataset
        fingerprint = compute_fingerprint(train_records, logger=logger)
        fp_path = save_fingerprint(fingerprint, str(out_root))
        logger.info(f"Fingerprint guardado en: {fp_path}")

        # 2. Generar plan de arquitectura
        gpu_vram = float(getattr(cfg, "gpu_vram_gb", 16.0))
        plan = generate_plan(fingerprint, gpu_vram_gb=gpu_vram, logger=logger)

        # Agregar overlap y fg oversampling al plan para uso en validación
        plan["patch_overlap"] = float(getattr(cfg, "patch_overlap", 0.5))
        plan["oversample_foreground"] = float(getattr(cfg, "oversample_foreground", 0.33))

        plan_path = save_plan(plan, str(out_root))
        logger.info(f"Plan de arquitectura guardado en: {plan_path}")

        # 3. Crear modelo desde el plan
        model = nnUNet3D.from_plan(plan, use_checkpointing=use_checkpointing)
        logger.info(
            f"Modelo auto-configurado: {model.get_num_parameters()/1e6:.3f}M params | "
            f"channels={plan['channels']} | strides={plan['strides']} | "
            f"patch={plan['patch_size']} | batch={plan['batch_size']}"
        )

        # 4. DataLoaders
        # Training: patch-based
        use_aug = bool(getattr(cfg, "augmentation", False))
        train_loader = make_patch_loader(train_records, plan, cfg, augment=use_aug)

        # Validación/test: volúmenes completos + sliding window
        ...
        logger.info("=" * 60)
        logger.info("MODO LEGACY (arquitectura fija)")
        logger.info("=" * 60)
        ...
        logger.info(f"Modelo legacy: {model.get_num_parameters()/1e6:.3f}M params")
    ...
    def ctor():
        if plan is not None:
            return nnUNet3D.from_plan(plan, use_checkpointing=use_checkpointing)
        return nnUNet3D(
            in_ch=1,
            base_ch=int(args.base_ch),
            levels=int(args.levels),
    ...
    if eval_ckpt:
        ...
        # Post-procesamiento de predicciones de validación
        if plan is not None and bool(getattr(cfg, "postprocess", True)):
            try:
                from postprocess import find_optimal_min_size, remove_small_components
                import numpy as np_pp

                logger.info("Buscando min_size óptimo para post-procesamiento...")
                predictions = []
                ground_truths = []
                for rec in val_records:
                    bin_path = os.path.join(str(preds_dir), f"{rec.study_id}_bin.npy")
                    if os.path.exists(bin_path):
                        predictions.append(np_pp.load(bin_path))
                        gt = np_pp.load(rec.mask_path)
                        if gt.ndim == 4:
                            gt = gt.squeeze()
                        ground_truths.append((gt > 0).astype(np_pp.uint8))

                if predictions:
                    best_min_size, best_pp_dice = find_optimal_min_size(predictions, ground_truths)
                    logger.info(f"Post-procesamiento: min_size={best_min_size} -> Dice={best_pp_dice:.4f}")

                    # Si mejora, aplicar a todas las predicciones
                    if best_min_size > 0:
                        for rec in val_records:
                            bin_path = os.path.join(str(preds_dir), f"{rec.study_id}_bin.npy")
                            if os.path.exists(bin_path):
                                pred = np_pp.load(bin_path)
                                filtered = remove_small_components(pred, min_size=best_min_size)
                                np_pp.save(bin_path, filtered.astype("float32"))
                        logger.info(f"Predicciones de validación post-procesadas con min_size={best_min_size}")

                        # Guardar min_size para usar en test
                        with open(out_root / "postprocess_config.json", "w") as f:
                            json.dump({"min_size": best_min_size, "val_dice_pp": best_pp_dice}, f, indent=2)
            except Exception as e:
        ...
        if test_loader is not None:
            try:
                ...
                # Post-procesar test si se determinó min_size en validación
                pp_config_path = out_root / "postprocess_config.json"
                if pp_config_path.exists():
                    try:
                        from postprocess import remove_small_components
                        import numpy as np_pp
                        with open(pp_config_path) as f:
                            pp_cfg = json.load(f)
                        pp_min_size = pp_cfg.get("min_size", 0)
                        if pp_min_size > 0:
                            for rec in test_records:
                                bin_path = os.path.join(str(test_preds_dir), f"{rec.study_id}_bin.npy")
                                if os.path.exists(bin_path):
                                    pred = np_pp.load(bin_path)
                                    filtered = remove_small_components(pred, min_size=pp_min_size)
                                    np_pp.save(bin_path, filtered.astype("float32"))
                            logger.info(f"Predicciones de test post-procesadas con min_size={pp_min_size}")
                    except Exception:
                        pass
    ...
    if auto_plan and plan:
        logger.info(f"Modo: AUTO-CONFIGURACIÓN")
        logger.info(f"Patch size: {plan['patch_size']}")
        logger.info(f"Strides: {plan['strides']}")
        logger.info(f"Canales: {plan['channels']}")
    else:
        logger.info(f"Modo: LEGACY")
    if val_metrics:
