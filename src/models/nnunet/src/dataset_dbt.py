...
def build_foreground_sampler(dataset: DBTVolumeDataset, oversample_ratio: float = 0.5) -> WeightedRandomSampler:
    ...
    if n_fg == 0 or n_bg == 0:
        # Fallback: sampling uniforme
        return None

    # Calcular pesos para que ~oversample_ratio de los batches tengan fg
...
class PatchDBTDataset(Dataset):
    """
    Dataset basado en patches para entrenamiento con nnU-Net auto-configurado.
    Extrae patches aleatorios de tamaño fijo del volumen completo,
    con oversampling de foreground.
    """
    def __init__(
        self,
        records: List[StudyRecord],
        patch_size: Tuple[int, int, int],
        force_fg_prob: float = 0.33,
        samples_per_epoch: Optional[int] = None,
        augment: bool = False,
        cfg=None,
        logger: Optional[logging.Logger] = None,
    ):
        from patch_utils import extract_patch, precompute_fg_coords

        self.records = records
        self.patch_size = tuple(patch_size)
        self.force_fg_prob = force_fg_prob
        self.augment = augment
        self.cfg = cfg
        self.logger = logger or get_logger("patch_dataset")
        self._extract_patch = extract_patch

        # Muestras por época: por defecto = número de estudios * 2
        # (para ver cada estudio ~2 veces con patches distintos)
        self.samples_per_epoch = samples_per_epoch or max(len(records) * 2, 250)

        # Pre-computar coordenadas de foreground para cada estudio
        self.fg_coords_cache: List[Optional[np.ndarray]] = []
        self.has_foreground: List[bool] = []
        for rec in records:
            coords = precompute_fg_coords(rec.mask_path)
            has_fg = len(coords) > 0
            self.fg_coords_cache.append(coords if has_fg else None)
            self.has_foreground.append(has_fg)

        n_fg = sum(self.has_foreground)
        self.logger.info(
            f"PatchDataset: {len(records)} estudios, {n_fg} con foreground | "
            f"patch={self.patch_size} | samples/epoch={self.samples_per_epoch}"
        )
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        # Elegir un estudio aleatorio
        rec_idx = idx % len(self.records)
        rec = self.records[rec_idx]

        img = np.load(rec.img_path).astype(np.float32)
        mask = np.load(rec.mask_path)
        ...
        # Decidir si forzar foreground
        force_fg = (np.random.rand() < self.force_fg_prob) and self.has_foreground[rec_idx]
        fg_coords = self.fg_coords_cache[rec_idx]

        # Extraer patch
        patch_img, patch_mask = self._extract_patch(
            img, mask, self.patch_size, force_fg=force_fg, fg_coords=fg_coords
        )

        # Augmentación
        if self.augment:
            patch_img, patch_mask = apply_augmentations(patch_img, patch_mask, self.cfg)

        # Tensor (1, Pz, Py, Px)
        img_t = torch.from_numpy(np.ascontiguousarray(patch_img)).unsqueeze(0).float()
        mask_t = torch.from_numpy(np.ascontiguousarray(patch_mask)).unsqueeze(0).float()

        return {
            "image": img_t,
            "mask": mask_t,
            "study_ids": [rec.study_id],
            "shapes": [tuple(patch_img.shape)],
            "norm_bounds": [(lo, hi)],
            "paths": [(rec.img_path, rec.mask_path)],
        }


def collate_patches(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Collate trivial para PatchDBTDataset: todos los patches tienen el mismo tamaño.
    """
    ...
    paths = []
    for b in batch:
        study_ids.extend(b["study_ids"])
        shapes.extend(b["shapes"])
        norm_bounds.extend(b["norm_bounds"])
...
def make_dataloaders(cfg) -> Tuple[DataLoader, DataLoader, List[StudyRecord], List[StudyRecord]]:
    """
    Descubre estudios, hace split 85/15 por estudio y retorna DataLoaders.
    Soporta foreground oversampling y augmentación.

    Returns
    -------
    train_loader, val_loader, train_records, val_records
    """
    ...
    # Split por estudio (determinístico)
    ...
    tr_loader = DataLoader(
        ...
        shuffle=(sampler is None),  # No shuffle si hay sampler
        sampler=sampler,
